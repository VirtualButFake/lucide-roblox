...
table_find = table_find
	or function(inputTable, valueToFind) -- Ignoring the `init` arg, unneeded for us
	...
	do
		...
		local function Args(...)
			...
			for _, Arg in next, { ... } do
				table_insert(EncodedValues, TypeCase(Type(Arg), Arg))
			end
	...
	for Key, Value in next, inputTable do
		...
		if TypeCases[KeyType] and TypeCases[ValueType] then
			...
			if KeyEncodedSuccess and ValueEncodedSuccess and EncodedValueOrError then
				...
				local KeyValue = EncodedKeyOrError
						and ((DontEncloseInBrackets and EncodedKeyOrError) or string_format("[%s]", EncodedKeyOrError)) .. ((Prettify and " = ") or "=")
					or ""
